...
fn onAccept(eventer: *Eventer, callback: *Eventer.Callback) anyerror!void {
    ...
    std.debug.print("accepting client on socket {}!\n", .{fd});
...

    {
        ...
        //std.debug.print("[DEBUG] new client at 0x{x}\n", .{@ptrToInt(newClient)});
        newClient.* = Client {
            .callback = Eventer.Callback {
                .func = onClientData,
...
fn onClientData(eventer: *Eventer, callback: *Eventer.Callback) anyerror!void {
    const fd = callback.data.fd;
    //std.debug.print("got data on socket {}!\n", .{fd});
    ...
    if (len == 0) {
        std.debug.print("client {} closed because read returned 0\n", .{fd});
        try removeClient(eventer, callback);
        return;
    }
    std.debug.print("[DEBUG] got {} bytes from socket {}\n", .{len, fd});
}
