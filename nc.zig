...
pub fn main() !u8 {
    ...
    global.sockfd = initSock: {
        if (optionalListenPort) |listenPort| {
            ...
            const listenFd = try posix.socket(addr.any.family, posix.SOCK.STREAM, posix.IPPROTO.TCP);
            defer posix.close(listenFd);
            if (builtin.os.tag != .windows) {
                try posix.setsockopt(listenFd, posix.SOL.SOCKET, posix.SO.REUSEADDR, &std.mem.toBytes(@as(c_int, 1)));
            }
            try posix.bind(listenFd, &addr.any, addr.getOsSockLen());
            try posix.listen(listenFd, 1);
            ...
            const clientFd = try posix.accept(listenFd, &clientAddr.any, &clientAddrLen, 0);
...
fn sockDisconnected() noreturn {
    ...
    posix.exit(0); // nothing else we can do
}
...
fn onSockData(eventer: *Eventer, callback: *Eventer.Callback) anyerror!void {
    ...
    const length = posix.read(global.sockfd, &global.buffer) catch |e| {
...
fn stdinClosed(eventer: *Eventer) !void {
    ...
    posix.close(global.stdin); // TODO: I don't have to close stdin...should I?
...
fn onStdinData(eventer: *Eventer, callback: *Eventer.Callback) anyerror!void {
    _ = callback;
    // TODO: I should use the sendfile syscall if available
    const length = posix.read(global.stdin, &global.buffer) catch |e| {
