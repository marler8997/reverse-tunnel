...
pub fn main() anyerror!u8 {
    ...
    var serverCallback = initServer: {
        ...
        std.debug.print("[DEBUG] server socket is {}\n", .{global.listenFd});
...
fn onAccept(eventer: *Eventer, callback: *Eventer.Callback) anyerror!void {
    ...
    const newsockfd = try posix.accept(global.listenFd, &addr.any, &addrlen, posix.SOCK.NONBLOCK);
    ...
    var info = clientInit: {
        ...
        std.debug.print("s={} closing connection from {}, already have 2 clients\n", .{newsockfd, addr});
        common.shutdownclose(newsockfd);
        return;
    };

    std.debug.print("s={} new client from {}\n", .{newsockfd, addr});
...
fn onDataOneClient(eventer: *Eventer, callback: *Eventer.Callback) anyerror!void {
    ...
    std.debug.print("s={} connection closed\n", .{clientRef.fd});
...
fn in_forward_from_closed(eventer: *Eventer, from: *Client, to: *Client) !void {
    ...
    if (!global.clientsLinked) {
        std.debug.print("client's weren't linked, back to one-client s={}\n", .{to.fd});
        ...
        std.debug.print("client's were linked, disconnecting s={}\n", .{to.fd});
...
fn forward(eventer: *Eventer, from: *Client, to: *Client) !void {
    const length = posix.read(from.fd, &global.buffer) catch |e| {
    ...
    std.debug.print("s={} forwarding {} bytes to {}\n", .{from.fd, length, to.fd});
    const sendResult = posix.send(to.fd, global.buffer[0..length], 0) catch |e| {
        std.debug.print("send on {} failed: {}\n", .{to.fd, e});
        std.debug.print("TODO: implement cleanup...\n", .{});
        return error.NotImplemented;
    };
    if (sendResult != length) {
        std.debug.print("only sent {} out of {} on {}\n", .{sendResult, length, to.fd});
        std.debug.print("TODO: implement something here...\n", .{});
        return error.NotImplemented;
    }
    if (!global.clientsLinked) {
        std.debug.print("s={} linked to s={}\n", .{from.fd, to.fd});
...
fn onDataClosing(eventer: *Eventer, callback: *Eventer.Callback) anyerror!void {
    const clientRef = callbackToClient(callback);
    std.debug.print("s={} finishing close\n", .{clientRef.fd});
    eventer.remove(clientRef.fd);
    posix.close(clientRef.fd);
    clientRef.fd = INVALID_FD;
}
